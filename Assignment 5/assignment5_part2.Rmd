...
library('readxl')
library("tidyverse")
df = read_excel("C:/Users/student/OneDrive - Bryant University/Desktop/Stats w R/MATH421/Data/c2015.xlsx")
```

2. Let's study the variable `SEX`. How many missing values in the NAs form? 
```{r}
table(df$SEX)
#Not Rep + Unknown = 1358 "NAs" in total
```
...
df$TRAV_SP = str_replace(df$TRAV_SP, 'MPH', '')

#df$TRAV_SP = drop(df$TRAV_SP == "Greater")

df = df %>% 
  filter(!df$TRAV_SP == "Greater")
...
medianTRAV_SP = mean(df$TRAV_SP, na.rm = TRUE)
